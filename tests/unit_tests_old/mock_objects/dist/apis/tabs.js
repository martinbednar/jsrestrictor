            ...
            const tabs = {
                async create(createProperties, fake = {}) {
                    ...
                    if (_networkTab) {
                        ...
                        const requestPromises = await tabs._fakeWebRequest(tab, fake, url, fakeWebRequestOptions, fakeWebRequestRedirects, fakeWebRequestDontYield, fakeWebRequestError);
                ...
                async update(tabId, updateProperties, fake = {}) {
                    ...
                    if (updateProperties.url &&
                        ...
                        const fakeWebRequestDontYield = fake.options.webRequestDontYield || [];
                        const fakeWebRequestError = fake.options.webRequestError || false;
                        promises = await tabs._fakeWebRequest(tab, fake, updateProperties.url, fakeWebRequestOptions, fakeWebRequestRedirects, fakeWebRequestDontYield, fakeWebRequestError);
                    }
                    else {
                    ...
                    return _tabs.filter((tab) => {
                        return Object.keys(query).every(key => {
                            if (key === 'currentWindow') {
                                return ((tab.windowId === browser.windows.WINDOW_ID_CURRENT) ===
                                    query[key]);
                            }
                ...
                async _fakeWebRequest(tab, fake, url, fakeWebRequestOptions, fakeRedirects, fakeDontYield, fakeError) {
                    let promises = [];
                    if (browser.tabs.onUpdated.addListener.callCount &&
                    ...
                    if (browser.webRequest.onBeforeRequest.addListener.callCount &&
                        ...
                        if (Array.isArray(result)) {
                            fake.responses.webRequest.onBeforeRequest = fake.responses.webRequest.onBeforeRequest.concat(result);
                            promises = promises.concat(result);
                        }
                    ...
                    if (browser.webRequest.onBeforeSendHeaders.addListener.callCount &&
                        ...
                        if (Array.isArray(result)) {
                            fake.responses.webRequest.onBeforeSendHeaders = fake.responses.webRequest.onBeforeSendHeaders.concat(result);
                            promises = promises.concat(result);
                        }
                        await Promise.all(promises);
                    }
