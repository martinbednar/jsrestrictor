...
def get_page_logs_thread(my_driver, browser_type, with_jsr, site, site_number, logs_ready, ret_logs):
    logs = []
    #try:
...
def get_logs_thread(thread_mark, browser_type, top_sites, sites_offset):
    ...
    for top_site in top_sites:
        ...
        get_logs_without_jsr_thread = Process(target=get_page_logs_thread, args=(driver_without_jsr, browser_type, False, top_site, sites_offset + site_number, send_logs_without_jsr_pipe_ready, send_logs_without_jsr_pipe))
        get_logs_without_jsr_thread.start()

        get_logs_with_jsr_thread = Process(target=get_page_logs_thread, args=(driver_with_jsr, browser_type, True, top_site, sites_offset + site_number, send_logs_with_jsr_pipe_ready, send_logs_with_jsr_pipe))
