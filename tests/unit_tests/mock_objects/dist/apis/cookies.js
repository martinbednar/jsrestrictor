    ...
    return {
        fakeApi(browser) {
            ...
            const _cookieFilter = (cookie, filter) => Object.keys(filter).every(key => {
                if (key === 'url') {
                    const parsedUrl = new url_1.URL(filter[key]);
                    if (!parsedUrl) {
            ...
            const cookies = {
                async set(details) {
                    ...
                    const parsedUrl = new url_1.URL(details.url);
                    if (!parsedUrl) {
                        throw new Error('Invalid url');
                    }
                ...
                async remove(details) {
                    const cookieIndex = _cookies.findIndex((cookie) => _cookieFilter(cookie, details));
                    if (cookieIndex === -1) {
                        return null;
                    }
                ...
                async get(query) {
                    const cookies = _cookies.filter((cookie) => _cookieFilter(cookie, query));
                ...
                async getAll(query) {
                    return _cookies.filter((cookie) => _cookieFilter(cookie, query));
                },
